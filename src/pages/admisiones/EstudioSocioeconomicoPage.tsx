...
import { EstudioSocioeconomicoForm } from './EstudioSocioeconomicoForm';
import { FileText, ArrowLeft, UserCircle } from 'lucide-react';
import apiClient from '../../services/api';

const EstudioSocioeconomicoPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPaciente = async () => {
      try {
        ...
        console.error('Error fetching patient for socioeconomic study:', error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
          <div>
            ...
            >
              <ArrowLeft size={18} /> Volver al CRM
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <div style={{ padding: '0.75rem', backgroundColor: '#10b981', color: 'white', borderRadius: '14px' }}>
                <FileText size={32} />
              </div>
              <div>
                <h1 style={{ fontSize: '24px', fontWeight: '900', color: '#1e293b', margin: 0 }}>Estudio Socioeconómico</h1>
                <p style={{ color: '#64748b', fontSize: '14px', margin: 0 }}>Tabulación de Cuotas y Entorno Familiar</p>
              </div>
            </div>
          </div>
            ...
            <div style={{ backgroundColor: 'white', padding: '1rem 1.5rem', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '1rem', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#64748b' }}>
                  {paciente.sexo === 'M' ? 'Hombre' : 'Mujer'} • {paciente.curp || 'Sin CURP'}
                </div>
              </div>
            </div>
          )}
...
export default EstudioSocioeconomicoPage;
