...
const AdmisionesDashboard: React.FC = () => {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1600px', margin: '0 auto', backgroundColor: '#f8fafc', minHeight: '100vh' }}>
      ...
      {/* BANNER: Pacientes listos para ingreso formal */}
      {(() => {
        const listos = pacientes.filter((p: any) => p.estado === 'PENDIENTE_INGRESO');
        ...
        return (
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.25rem' }}>
              ...
              <span style={{ fontWeight: '900', fontSize: '15px', color: '#15803d' }}>
                {listos.length} paciente{listos.length > 1 ? 's' : ''} listo{listos.length > 1 ? 's' : ''} para ingreso formal
              </span>
            </div>
            {listos.map((p: any) => (
              <div
              ...
              >
                <span style={{ fontSize: '14px', fontWeight: '700', color: '#166534' }}>
                  Paciente #{p.claveUnica || p.id} — {p.nombre} {p.apellidoPaterno}
                </span>
                <button
                  onClick={() => navigate(`/admisiones/nueva-solicitud?pacienteId=${p.id}`)}
                  style={{
                    padding: '0.4rem 1rem',
