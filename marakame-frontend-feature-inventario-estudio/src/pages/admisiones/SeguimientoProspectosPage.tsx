...
export default function SeguimientoProspectosPage() {
  ...
  const mutationSolicitar = useMutation({
    ...
    onSuccess: () => {
      // Actualiza el CRM de admisiones y la bandeja del médico simultáneamente
      queryClient.invalidateQueries({ queryKey: ['prospectos_crm'] });
                  ...
                  <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '1.25rem 2rem', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'center' }}>
                        ...
                        {(() => {
                          const estado = p.paciente?.estado;
                          if (estado === 'PENDIENTE_INGRESO') {
                            return (
                              <button
                                onClick={() => navigate(`/admisiones/nueva-solicitud?pacienteId=${p.paciente?.id}`)}
                                title="Diagnóstico completado — continuar con ingreso formal"
                                style={{
                              ...
                              >
                                <Stethoscope size={13} />
                                <span>Continuar Ingreso</span>
                              </button>
                            );
                          }
                          const yaEnValoracion = estado === 'EN_VALORACION';
                          return (
                            <button
                              onClick={() => !yaEnValoracion && handleSolicitarValoracion(p)}
                              title={yaEnValoracion ? 'En Revisión Médica' : 'Solicitar Valoración Médica'}
                              disabled={yaEnValoracion}
                              style={{
                                padding: '0.5rem',
                                borderRadius: '8px',
                                border: `1px solid ${yaEnValoracion ? '#fde68a' : '#e2e8f0'}`,
                                backgroundColor: yaEnValoracion ? '#fefce8' : 'white',
                                cursor: yaEnValoracion ? 'not-allowed' : 'pointer',
                                color: yaEnValoracion ? '#92400e' : '#10b981',
                                opacity: yaEnValoracion ? 0.8 : 1,
                                display: 'flex', alignItems: 'center', gap: '0.25rem',
                                fontSize: '11px', fontWeight: '700'
                              }}
                            >
                              <Stethoscope size={14} />
                              {yaEnValoracion && <span>En Revisión</span>}
                            </button>
                          );
                        })()}
